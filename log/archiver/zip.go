...
func (zipFormat) Make(destination string, sources []string) error {
...
func zipFile(w *zip.Writer, file string) error {
	...
	if err != nil {
		return fmt.Errorf("%s: stat: %v", file, err)
	}

	header, err := zip.FileInfoHeader(info)
	if err != nil {
		return fmt.Errorf("%s: getting header: %v", file, err)
	}
	header.Name = file
	...
	if err != nil {
		return fmt.Errorf("%s: making header: %v", file, err)
	}
	...
	if header.Mode().IsRegular() {
		...
		if err != nil {
			return fmt.Errorf("%s: opening: %v", file, err)
		}
		defer file.Close()

		_, err = io.CopyN(writer, file, info.Size())
		if err != nil && err != io.EOF {
			return fmt.Errorf("%s: copying contents: %v", file, err)
		}
	}
